...
stage_1_data_old <- function(mu, corr, n) {
    ...
    X1=X1+mu[1:5]
    t1=(rowMeans(X1[2:5,])-mean(X1[1,]))/sqrt((rowVars(X1[2:5,])+var(X1[1,]))/n)
    t2=(rowMeans(X2[2:5,])-mean(X2[1,]))/sqrt((rowVars(X2[2:5,])+var(X2[1,]))/n)
...
gen_data <- function(n, corr_control, corr_treatment, cont_treat_association, eff, n_cont, n_treat) {
  ...
  cont_var <- colVars(control, na.rm=T, dim. = c(max(n_cont), length(n_cont) * n))
  dim(cont_var) <- dim(control)[-1]
  treat_var <- colVars(treatment, na.rm=T, dim. = c(max(n_treat), length(n_treat) * n))
...
gen_data_naive <- function(n, corr_control, corr_treatment, cont_treat_association, eff, n_cont, n_treat) {
  if (length(n_cont) == 1) {
    n_cont <- rep(n_cont, dim(corr_control)[1])
  }
  if (length(n_treat) == 1) {
...
stage_2_data_old <- function(mu, corr, n2, trt2, runs2) {
    ...
    Y2= corr*Y1+sqrt(1-corr^2)*array(rnorm(mtn2*(ntrt+1)*runs2),dim=c(ntrt+1,mtn2,runs2))+mu[c(6,trt2+6)] # secondary endpoint data
    Y1=Y1+mu[c(1,trt2+1)]
    ...
    t1 = (t(my1[-1,,drop=FALSE]) - my1[1,]) / sqrt(
      ((t(vy1[-1,,drop=FALSE] * (tn2s[-1] - 1)) + vy1[1,] * (tn2s[1] - 1)) / (tn2s[1] + tn2s[-1] - 2)) * 
        (1 / tn2s[1] + 1 / tn2s[-1])
    )
    t2 = (t(my2[-1,,drop=FALSE]) - my2[1,]) / sqrt(
      ((t(vy2[-1,,drop=FALSE] * (tn2s[-1] - 1)) + vy2[1,] * (tn2s[1] - 1)) / (tn2s[1] + tn2s[-1] - 2)) * 
        (1 / tn2s[1] + 1 / tn2s[-1])
    )
...
mu <- c(eff, eff)
corr <- 0.8
p_val_old <- as_tibble(do.call(rbind, (lapply(1:20000, function(i) {
  as.data.frame(list(i=i, val=stage_1_data_old(mu, corr, 50), index=paste0("H",1:8)))
  }
))))
...
p_long <- as_tibble(
  gen_data(20000, corr_control, corr_treatment, cont_treat_association, eff, 50, 50),
  .name_repair = "universal"
)
