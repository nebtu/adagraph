...
simrun <- function(design,
  ...
  X1=X1+mu[1:5]
  t1=(rowMeans(X1[2:5,])-mean(X1[1,]))/sqrt((matrixStats::rowVars(X1[2:5,])+var(X1[1,]))/n1)
  t2=(rowMeans(X2[2:5,])-mean(X2[1,]))/sqrt((matrixStats::rowVars(X2[2:5,])+var(X2[1,]))/n1)
  ...
  if (futility > 0) {
    ...
    drop_hyp <- (design_interim$p_values_interim[1:4] != min(design_interim$p_values_interim[1:4][!trt_rej]))
  }
  ...
  if (any(!drop_hyp)) {
    ...
    fwer <- mean(matrixStats::colMaxs(sapply(designs_final, function(x) as.integer(x$rej))))
...
sim_wrap <- function(design, eff=c(0,0.25, 0.25,0.25,0.25), futility=futility_v, runs1=100, runs2=100, corr=0.5) {
  mu = c(eff, eff)
  results = lapply(1:runs1, function(i) simrun(design, mu=mu, corr=corr, runs2=runs2, n1=50, n2=50, futility=futility))
