#' Adapt the patient numbers for the second stage of a MAME design
#'
...
#'
#' @param design The `mame_design` to be adapted
...
#'   there are no subgroups
#' @param ad_n_subgroups A data.frame specifying the structure of the subgroups,
...
#'   after.
#'
#' @return An object of class `mame_design`, with the adaptions applied.
#'
#' @examples
#' m <- rbind(
#'   c(0, 1 / 2, 1 / 2, 0),
...
mame_adapt_n <- function(
  ...
  if (!is.null(n_control_2) || !is.null(n_arms_2)) {
    ...
    } else if (!is.null(ad_n_subgroups)) {
      cli::cli_warn(
        "{.arg n_control_2} and {.arg n_arms_2} are ignored since {.arg ad_n_subgroups} was supplied"
      )
    } else {
      ad_n_subgroups <- data.frame(
  ...
  design_ad$ad_n_subgroups <- ad_n_subgroups

  design_ad
}

#' Dropp groups, arms or endpoints from a MAME design
#'
...
#' @name mame_drop
#'
#' @param design The `mame_design` of which to drop the
