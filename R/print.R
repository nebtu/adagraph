...
print.cer_design <- function(design) {
  ...
  if (design$interim_test) {
    ...
    cli::cat_line("P-values of interim test are ", design$p_values_interim)
  ...
  if (design$adaptions) {
    ...
    cli::cat_line("")
    if (design$ad_weight != design$weights) {
      cli::cat_line("New hypotheses weights")
      cli::cat_print(design$ad_weights)
      cli::cat_line()
    }
    if (design$ad_test_m != design$test_m) {
      cli::cat_line("New graph Transition Matrix")
      cli::cat_print(design$ad_test_m)
      cli::cat_line()
    }
    if (design$ad_correlation != design$correlation) {
      cli::cat_line("New correlation for parametric test")
      cli::cat_print(design$adcorrelation)
      cli::cat_line()
    }
    if (design$ad_t != design$t) {
      cli::cat_line("Interim test was done at time fraction ", design$t)
      cli::cat_line("")
    }
  }
}
